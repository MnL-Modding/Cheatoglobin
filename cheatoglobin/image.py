...
def create_MObj_sprite(table_offsets, overlay, MObj_file, group_num, anim_num, lang):
    ...
    overlay.seek(table_offsets[0] + (anim_id * 4))
    MObj_file.seek(int.from_bytes(overlay.read(4), "little"))
    animation_file = BytesIO(decompress(MObj_file))

    overlay.seek(table_offsets[0] + (graph_id * 4))
    MObj_file.seek(int.from_bytes(overlay.read(4), "little"))
    graphics_buffer = BytesIO(decompress(MObj_file))

    overlay.seek(table_offsets[0] + (pal_id * 4))
    MObj_file.seek(int.from_bytes(overlay.read(4), "little"))
    palette_file_size = int.from_bytes(MObj_file.read(4), "little")
    palette_file = define_palette(struct.unpack(f'<{palette_file_size // 2}H', MObj_file.read(palette_file_size)))
